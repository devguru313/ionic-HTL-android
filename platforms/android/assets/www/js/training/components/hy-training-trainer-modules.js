  ...
  return {
    ...
    controller: function ($scope, $rootScope, $state, $timeout, $stateParams, $sce, $ionicScrollDelegate, TrainingStore, TrainerModule, Module) {
      ...
      $scope.init = function() {
        ...
        console.log("trainer module"+ $scope.trainerIds);
        console.log(JSON.stringify(TrainingStore.getTrainers()));
        ...
        var checkscreenloaded = setInterval(function(){

            $scope.init();
            if(TrainingStore.getTrainers()){
            stoploadifloaded()
            }}, 500);

         function stoploadifloaded() {
           clearInterval(checkscreenloaded);
         }
